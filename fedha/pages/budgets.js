...
export default function BudgetsPage() {
  const { budgets, loans, incomePlans, wallets, addBudget, updateBudget, removeBudget, addLoan, updateLoan, removeLoan, addIncomePlan, updateIncomePlan, removeIncomePlan, addTransaction, removeTransaction, currency } = useApp();
                ...
                {incomePlans.map((p) => {
                  ...
                  return (
                    <div key={p.id} className="card" style={{ padding: '16px', borderColor: p.is_received ? 'rgba(16,185,129,0.3)' : undefined }}>
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button onClick={() => toggleIncomeReceived(p)} style={{ flex: 1, padding: '8px', background: p.is_received ? 'var(--green-dim)' : 'var(--card-2)', border: `1px solid ${p.is_received ? 'var(--green)' : 'var(--border)'}`, borderRadius: 8, color: p.is_received ? 'var(--green)' : 'var(--text-2)', fontSize: 13, cursor: 'pointer', fontFamily: 'Outfit', fontWeight: 600 }}>
                ...
                {activeLoans.map((l) => {
                  ...
                  return (
                    <div key={l.id} className="card" style={{ padding: '16px', borderColor: overdue ? 'rgba(239,68,68,0.3)' : undefined }}>
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button onClick={() => settleLoan(l)} style={{ flex: 1, padding: '8px', background: 'var(--green-dim)', border: '1px solid rgba(16,185,129,0.3)', borderRadius: 8, color: 'var(--green)', fontSize: 13, cursor: 'pointer', fontFamily: 'Outfit', fontWeight: 600 }}>✓ Mark Settled</button>
